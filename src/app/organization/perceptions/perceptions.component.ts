...
export class PerceptionsComponent implements OnInit {
  ...
  searchResult: SearchResults<Perception> = new SearchResults<Perception>();
  ...
  search() {
    ...
    //Send Request
    this.dataService.perceptions().search(this.organization, criteria).subscribe(
      result => {
