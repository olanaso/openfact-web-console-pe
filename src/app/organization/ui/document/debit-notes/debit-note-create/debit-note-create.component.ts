...
export class DebitNoteCreateComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() { 
    ...
    this.dataSubscription = this.route.data.subscribe((data) => {
      this.tiposNotaDebito = data['tiposNotaDebito'] || [];
  ...
  loadDataForm() {
    const formValue: any = {};
    if (this.tiposNotaDebito && this.tiposNotaDebito.length > 0) {
      formValue.tipoDeNotaDeDebito = this.tiposNotaDebito[0].codigo;
    }
    if (this.igv) {
    ...
    this.dialogService.confirm('Confirm', 'Estas seguro de realizar esta operacion').result.then(
      (redirect) => {
        ...
        this.dataService.organizationsSunat().createDebitNotes(this.organization.organization, form.value).subscribe(
          response => {
            this.working = false;
            this.toastr.success('Success! The debit note has been created.');
            if (redirect) {
