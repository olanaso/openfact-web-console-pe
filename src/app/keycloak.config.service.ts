...
const defaultConfigJson = '/config/keycloak.json';

export function keycloakConfigServiceInitializer(config: KeycloakConfigService) {
  return () => config.load();
}
...
export class KeycloakConfigService {
  ...
  load(configJson: string = defaultConfigJson): Promise<KeycloakConfigService> {
