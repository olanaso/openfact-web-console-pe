...
@Component({
  selector: 'of-voideds',
  templateUrl: './voideds.component.html',
  styleUrls: ['./voideds.component.scss']
})
export class VoidedsComponent implements OnInit {

  organization: Organization;
  searchResult: SearchResults<Voided> = new SearchResults<Voided>();
  ...
  search() {
    ...
    //Send Request
    this.dataService.voideds().search(this.organization, criteria).subscribe(
      result => {
        this.searchResult = result;
      }, error => {
        this.alertService.pop('error', 'Error', 'Error loading voideds.');
      });
  }
  downloadXml(voided: any) {
    let ret: Voided = this.dataService.voideds().build(this.organization, voided.codigoUnico);
    voided.downloadXml(ret);
  }

  downloadPdf(voided: any) {
    let ret: Voided = this.dataService.voideds().build(this.organization, voided.codigoUnico);
    voided.downloadPdf(ret);
  }

  sendToCustomer(voided: any) {
    let ret: Voided = this.dataService.voideds().build(this.organization, voided.codigoUnico);
    voided.sendToCustomer(ret).subscribe(
      result => {
  ...
  sendToThirdParty(voided: any) {
    let ret: Voided = this.dataService.voideds().build(this.organization, voided.codigoUnico);
    voided.sendToThirdParty(ret).subscribe(
      result => {
