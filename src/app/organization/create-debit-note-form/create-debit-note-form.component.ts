...
@Component({
  selector: 'of-create-debit-note-form',
  templateUrl: './create-debit-note-form.component.html',
  styleUrls: ['./create-debit-note-form.component.scss']
})
export class CreateDebitNoteFormComponent implements OnInit, OnDestroy {
  ...
  save(form: any): void {
    ...
    const modalRef = this.modalService.open(CreateDebitNoteFormConfirmModalComponent)
    ...
    modalRef.result.then((redirect) => {
      this.working = true;
      this.dataService.organizationPeru().createDebitNote(this.organization, form).subscribe(
        response => {
          this.working = false;
