...
export class RetentionsComponent implements OnInit {
  ...
  searchResult: SearchResults<Retention> = new SearchResults<Retention>();
  ...
  combo = {
    ...
    orderBy: [
      ...
      { name: 'Type Code', value: 'retentionTypeCode' }
    ]
  };
  ...
  search() {
    ...
    //Send Request
    this.dataService.retentions().search(this.organization, criteria).subscribe(
      result => {
